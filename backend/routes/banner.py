...
# ============================================================
# POST /banners/ — Upload new banner
# IMPORTANT: Router prefix is /banners so this handles POST /banners/
# ============================================================
...
async def upload_banner(
    ...
    file_path = storage.upload(content, unique_filename, folder=f"banners/{current_user['id']}")
...
# ============================================================
# IMPORTANT: Static/specific routes MUST come BEFORE /{banner_id}
# to prevent FastAPI from matching "active", "all", "stats", "file"
# as banner_id path params.
# ============================================================
...
async def get_overall_banner_stats(
    ...
    return {
        ...
        "overall_ctr": ctr,
        "avg_ctr": ctr  # alias for frontend compatibility
    }
...
async def update_banner(
    ...
    if background_color is not None: update_data["background_color"] = background_color
    if text_color is not None: update_data["text_color"] = text_color
    if button_color is not None: update_data["button_color"] = button_color
    if button_text_color is not None: update_data["button_text_color"] = button_text_color
    try:
...
@router.post("/{banner_id}/toggle", summary="Toggle banner active status")
async def toggle_banner_status(
    ...
    return {"message": "Banner status toggled", "is_active": new_status}
...
async def delete_banner(banner_id: str, current_user: dict = Depends(role_required(["admin"]))):
