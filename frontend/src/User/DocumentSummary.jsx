...
const DocumentSummary = () => {
  ...
  const getEventTitle = (event) => {
    // Use title from API if available, otherwise create from type
  ...
  if (isLoading) return (
    <div className="doc-summary-loading">
      <div className="doc-summary-spinner"></div>
  ...
  if (!docData || !docStats) return null;

  const currentStatus = statusConfig[docData.status] || statusConfig.draft;
      ...
      <header className="doc-simple-header">
        ...
        <div className="doc-simple-left">
          ...
          <button
            ...
            disabled={docData?.status !== "draft"}
          >
            <FiEdit size={16} />
            <span>Edit</span>
          </button>

          {/* <button className="doc-header-action">
        ...
        {/* RIGHT ACTIONS */}
          ...
          <div className="doc-simple-actions">
            <RefreshCw size={16} onClick={fetchDocumentSummary}/> 
...
/>
      ...
      {/* Quick Status Overview (for sidebar or summary) */}
      {/* <section>
  {documentRecipients.length > 0 && (
    <RecipientStatusBar
      recipient={documentRecipients[0]}
    ...
    />
      ...
      {/* ================= QUICK STATS CARDS ================= */}
      {/* <section className="doc-summary-stats-grid">
        <div 
          className="doc-summary-stat-card" 
          onClick={() => toggleSectionVisibility('recipients')}
        >
          <div className="doc-summary-stat-icon" style={{ color: '#3b82f6' }}>
            <Users size={20} />
          </div>
          <div className="doc-summary-stat-content">
            <h3>{docStats.total_recipients || 0}</h3>
            <p>Total Recipients</p>
          </div>
          <div className="doc-summary-stat-indicator">
            {activeSection === 'recipients' ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
          </div>
        </div>
        ...
        >
          <div className="doc-summary-stat-icon" style={{ color: '#10b981' }}>
            <CheckCircle size={20} />
          </div>
          <div className="doc-summary-stat-content">
            <h3>{docStats.signed_recipients || 0}</h3>
            <p>Completed</p>
          </div>
          <div className="doc-summary-stat-indicator">
            {activeSection === 'progress' ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
          </div>
        </div>
        ...
        >
          <div className="doc-summary-stat-icon" style={{ color: '#8b5cf6' }}>
            <History size={20} />
          </div>
          <div className="doc-summary-stat-content">
            <h3>{timelineEvents.length}</h3>
            <p>Activities</p>
          </div>
          <div className="doc-summary-stat-indicator">
            {activeSection === 'timeline' ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
          </div>
        </div>
        ...
        {/* <div 
          ...
          onClick={() => toggleSectionVisibility('recipients')}
        >
          <div className="doc-summary-stat-icon" style={{ color: '#f59e0b' }}>
            <UserCheck size={20} />
          </div>
          <div className="doc-summary-stat-content">
          ...
          <div className="doc-summary-stat-indicator">
            {activeSection === 'overview' ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
          </div>
        </div> 
      </section> */}
      ...
      {activeSection === 'overview' && (
        <section className="doc-summary-expanded-section">
          <div className="doc-summary-section-header">
            ...
            <button 
              className="doc-summary-section-close"
        ...
        <section className="doc-summary-expanded-section">
          <div className="doc-summary-section-header">
            <div className="doc-summary-section-title">
              ...
              <button 
                className="doc-summary-download-small"
                onClick={() => handleReportDownload('csv')}
                disabled={downloadInProgress === 'csv'}
              >
                <Download size={14} />
                {downloadInProgress === 'csv' ? '...' : 'CSV'}
              </button>
            </div>
            <button 
              className="doc-summary-section-close"
          ...
          <div className="doc-summary-recipients-grid">
            {documentRecipients.map((recipient) => {
              ...
              const lastAction = recipient.signed_at || recipient.approved_at || 
                               recipient.viewer_at || recipient.form_completed_at || 
                               recipient.witnessed_at || 'No action yet';
              
              return (
                <div key={recipient.id} className="doc-summary-recipient-card">
                  <div className="doc-summary-recipient-header">
                    <div className="doc-summary-recipient-avatar">
                      <Users size={16} />
                    </div>
                    ...
                    <span 
                      className="doc-summary-recipient-status" 
                      style={{ color: recipientStatus.color }}
                    >
                      {recipientStatus.icon} {recipientStatus.label}
                    </span>
                  </div>
                  
                  <div className="doc-summary-recipient-details">
                    <div className="doc-summary-recipient-detail">
                      <span>Role</span>
                      <strong>{recipient.role?.replace('_', ' ') || 'Signer'}</strong>
                    </div>
                    <div className="doc-summary-recipient-detail">
                      <span>Order</span>
                      <strong>{recipient.signing_order || 'N/A'}</strong>
                    </div>
                    <div className="doc-summary-recipient-detail">
                      <span>Last Action</span>
                      <small>
                        {lastAction !== 'No action yet' ? new Date(lastAction).toLocaleDateString() : lastAction}
                      </small>
                    </div>
                  </div>
                </div>
        ...
        <section className="doc-summary-expanded-section">
          <div className="doc-summary-section-header">
            <div className="doc-summary-section-title">
              ...
              <small>{timelineEvents.length} total activities • {new Set(timelineEvents.map(e => e.user)).size} users</small>
              <button 
                className="doc-summary-download-small"
                onClick={() => handleReportDownload('html')}
                disabled={downloadInProgress === 'html'}
              >
                <Download size={14} />
                {downloadInProgress === 'html' ? '...' : 'Report'}
              </button>
            </div>
            <button 
              className="doc-summary-section-close"
          ...
          <div className="doc-summary-timeline-container">
            {timelineEvents.length === 0 ? (
              <div className="doc-summary-empty-state">
                ...
                <small>Timeline events will appear here as you interact with the document</small>
              </div>
            ) : (
              <div className="doc-summary-timeline-list">
                {visibleTimelineEvents.map((event, index) => (
                  <div key={index} className="doc-summary-timeline-event">
                    <div className="doc-summary-timeline-icon" style={{ backgroundColor: getTimelineColor(event.type) + '20', color: getTimelineColor(event.type) }}>
                      {getTimelineIcon(event.type)}
                    </div>
                    <div className="doc-summary-timeline-content">
                      <div className="doc-summary-timeline-header">
                        <h4>{getEventTitle(event)}</h4>
                        <span className="doc-summary-timeline-time">
                      ...
                      <div className="doc-summary-timeline-footer">
                        ...
                        <small className="doc-summary-timeline-full-date">
                      ...
                      {event.metadata && Object.keys(event.metadata).length > 0 && (
                        <div className="doc-summary-timeline-metadata">
                          {event.metadata.filename && (
                            <span className="doc-summary-metadata-chip">
                              File: {event.metadata.filename}
                            </span>
                          )}
                          {event.metadata.envelope_id && (
                            <span className="doc-summary-metadata-chip">
                              Envelope: {event.metadata.envelope_id}
                            </span>
                          )}
            ...
            {timelineEvents.length > 0 && (
              <div className="doc-summary-timeline-actions">
                {timelineEvents.length > TIMELINE_LIMIT && (
                  <button
                    className="doc-summary-view-more"
                ...
                <div className="doc-summary-timeline-stats">
                  <div className="doc-summary-timeline-stat">
                    <span className="doc-summary-stat-label">First Event</span>
                    <span className="doc-summary-stat-value">
                      {timelineEvents.length > 0 ? 
                        new Date(timelineEvents[timelineEvents.length - 1].timestamp).toLocaleDateString() : 
                        'N/A'
                      }
                    </span>
                  </div>
                  <div className="doc-summary-timeline-stat">
                    <span className="doc-summary-stat-label">Last Updated</span>
                    <span className="doc-summary-stat-value">
                      {timelineEvents.length > 0 ? 
      ...
      <footer className="doc-summary-footer">
        <div className="doc-summary-footer-actions">
          {/* <button 
            className="doc-summary-refresh-btn"
            onClick={fetchDocumentSummary}
          >
        ...
        >
          ...
          <div className="doc-summary-export-options">
            <span>Export as:</span>
            <button onClick={() => handleReportDownload('json')} disabled={downloadInProgress === 'json'}>
              JSON
            </button>
            <button onClick={() => handleReportDownload('csv')} disabled={downloadInProgress === 'csv'}>
              CSV
            </button>
            <button onClick={() => handleReportDownload('zip')} disabled={downloadInProgress === 'zip'}>
              Full Export
            </button>
          </div>
        </div>
        <div className="doc-summary-footer-info">
